...
for project in "$PAGES_PROJECT" "$FORM_PROJECT"; do
  if wrangler pages project create "$project" --production-branch main >/dev/null 2>&1; then
...
JS
wrangler pages deploy "$form_tmp" --project-name "$FORM_PROJECT" --branch main --commit-dirty=true >/dev/null
